...
| `ui/viewmodels/SettingsViewModel` | SharedPreferences-backed settings (notifications, vibration, display name, dark mode, receive permission, bio). `setBio(bio)` persists 100-char bio string |
...
| `ui/viewmodels/SettingsUiStateTest` | Defaults, toggle copies, display name, dark mode, bio default + copy, equality (9 tests) |
...
| Settings SharedPrefs | `boop_settings` (keys: notifications_enabled, vibration_enabled, display_name, dark_mode_enabled, receive_permission, user_ulid, bio) |
