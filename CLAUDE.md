...
**Navigation model**: Flat stack — back stack never exceeds `[Home, <one screen>]`. Tab destinations (Home, History, Profile) use `popUpTo(startDestination) { saveState = true } + launchSingleTop + restoreState`. Overlay screens (TransferProgress, Settings) use `popUpTo(Home) { inclusive = false } + launchSingleTop`. Bottom nav hidden on TransferProgress, NfcGuide, Settings, and FriendProfile. **Slide animations**: `slideIntoContainer`/`slideOutOfContainer` with 300ms tween, direction based on `BoopRoute.tabIndex()` (Home=0, History=1, Profile=2, Settings/FriendProfile=3). Tab-to-tab slides left/right based on index difference; push screens slide in from right, pop slides out to right.
...
| `ui/viewmodels/ProfileViewModel` | AndroidViewModel for user's local profile: `profileItems` from Room, `profilePicPath` from SharedPreferences, add/update/delete/reorder items (max 6), profile pic copy to filesDir, `buildProfileJson()` outputs envelope `{items: [...], answers: {...}}`, `parseProfileJson()` returns `ParsedProfile(items, answers)` — backward-compatible with old `[...]` format |
| `ui/viewmodels/SettingsViewModel` | SharedPreferences-backed settings (notifications, vibration, display name, dark mode, receive permission, profile answers). `ProfileQuestion` data class + `PROFILE_QUESTIONS` list (3 predefined "About Me" questions). `setProfileAnswer(key, value)` persists answers as JSON |
...
- **Profile bento grid** — max 12 items, types: link/email/phone, sizes: half (1x1 icon-only square) / full (1x2 icon+label wide). 4-column grid layout with greedy row-packing. BentoGrid icons use native brand colors via `resolveSocialIconColor()` (Instagram pink, LinkedIn blue, YouTube red, Facebook blue; GitHub/Twitter white; others use accent fallback). Stored in Room `profile_items` table. Items serialized to JSON envelope `{items: [...], answers: {...}}` for wire transfer via `ProfileViewModel.buildProfileJson()` or `TransferViewModel.buildLocalProfile()` (reads from DB directly). Backward-compatible: old `[...]` format parsed as items-only with empty answers. Friend profiles parsed via `ProfileViewModel.parseProfileJson()` → `ParsedProfile(items, answers)`
- **Profile questions ("About Me")** — 3 predefined questions stored in SharedPreferences `profile_answers` as JSON map. Questions: `prefer_contact` ("I prefer" → Texting/Calling), `gc_permission` ("Add me to GCs without asking" → Yes/No), `reply_speed` ("I reply" → Fast/Slow). Defined as `PROFILE_QUESTIONS` list of `ProfileQuestion` data class. UI: individual GlassCard per question with FilterChip toggles on ProfileScreen. FriendProfileScreen shows read-only answered questions as purple pills. Answers included in wire format envelope and backup export/import
...
| `backup/BackupSerializerTest` | Serialize/deserialize round-trip (profile, friends, history), Base64 round-trip, empty data, null fields, future version, full crypto+serialization integration, profile answers round-trip, null answers backward compat (10 tests) |
| `ui/viewmodels/SettingsUiStateTest` | Defaults, toggle copies, display name, dark mode, profileAnswers default + copy, equality (9 tests) |
...
| `ui/viewmodels/ParsedProfileTest` | Profile JSON format parsing: old bare array, new envelope with answers, empty/null/blank, missing answers key (6 tests) |
...
| Settings SharedPrefs | `boop_settings` (keys: notifications_enabled, vibration_enabled, display_name, dark_mode_enabled, receive_permission, user_ulid, profile_answers) |
